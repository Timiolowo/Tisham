  ...
  try {
    ...
    if (role === 'school_admin') {
      ...
      const { data: school, error: schoolError } = await supabase
        .from('schools')
        .insert({ name: schoolName, type: schoolType, state, address, contact_email: contactEmail, contact_phone: contactPhone, admin_name: adminName })
        .select()
        .single();
      ...
      if (schoolError) {
        ...
        return {
          ...
          body: JSON.stringify({ error: schoolError.message }),
        };
      }
